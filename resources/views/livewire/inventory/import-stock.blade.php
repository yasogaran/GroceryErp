<div class="p-6">
    <!-- Header -->
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Import Stock</h2>
        <p class="text-gray-600 mt-1">Upload a CSV file to import initial stock for multiple products at once</p>
    </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <ul class="text-sm text-gray-700 space-y-1 list-disc list-inside">
                    <li><strong>product_sku</strong> (optional): Product SKU for identification</li>
                    <li><strong>product_name</strong> (required): Product name (must exist in database)</li>
                    <li><strong>quantity</strong> (required): Stock quantity to add</li>
                    <li><strong>unit_cost</strong> (required): Cost per unit</li>
                    <li><strong>supplier_name</strong> (optional): Supplier name</li>
                    <li><strong>batch_number</strong> (optional): Batch/Lot number</li>
                    <li><strong>manufacturing_date</strong> (optional): Manufacturing date (YYYY-MM-DD)</li>
                ...
                <div class="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded">
                    <p class="text-sm text-yellow-800"><strong>Note:</strong> Make sure the products exist before importing stock. Create products first if needed.</p>
                </div>
            </div>
            ...
            <div class="flex space-x-3">
                ...
                <a href="{{ route('stocks.index') }}" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition">
                    Cancel
                </a>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($validationResults['valid_rows'] as $row)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-3 py-2">
                                            <div class="text-gray-900">{{ $row['data']['product_name'] }}</div>
                                            @if($row['data']['product_sku'])
                                                <div class="text-xs text-gray-500">SKU: {{ $row['data']['product_sku'] }}</div>
                                            @endif
                                        </td>
                                        <td class="px-3 py-2 text-gray-900">{{ $row['data']['quantity'] }}</td>
                                        <td class="px-3 py-2 text-gray-900">₹{{ number_format($row['data']['unit_cost'], 2) }}</td>
                                        <td class="px-3 py-2 text-gray-600">{{ $row['data']['supplier_name'] ?: '-' }}</td>
                                        <td class="px-3 py-2 text-gray-600">{{ $row['data']['batch_number'] ?: '-' }}</td>
                                        <td class="px-3 py-2 text-gray-600">{{ $row['data']['expiry_date'] ?: '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                        ...
                        @foreach($validationResults['invalid_rows'] as $row)
                            <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                                ...
                                <div class="text-sm text-red-800 mb-2">
                                    <strong>Product:</strong> {{ $row['data']['product_name'] ?? 'N/A' }}
                                    @if(!empty($row['data']['product_sku']))
                                        (SKU: {{ $row['data']['product_sku'] }})
                                    @endif
                                    , <strong>Quantity:</strong> {{ $row['data']['quantity'] ?? 'N/A' }}
                                </div>
            ...
            <div class="flex space-x-3">
                <button wire:click="import" wire:loading.attr="disabled"
                    ...
                    <span wire:loading.remove wire:target="import">Import Stock for {{ $validationResults['valid_count'] }} Products</span>
    ...
    @if($step === 3)
        <div class="bg-white rounded-lg shadow p-6">
            <div class="text-center py-8">
                ...
                <p class="text-gray-600 mb-6">Stock has been imported successfully</p>
                ...
                <div class="flex justify-center space-x-3">
                    <a href="{{ route('stocks.index') }}" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                        View Stock
                    </a>
                    <button wire:click="reset" type="button" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition">
                        Import More
